...
│   ├── chrome-profile.ts     ← CDP Chrome profile dir resolver — opt-in PI_WEBAIO_CHROME_PROFILE_DIR override, fail-closed validation (#127)
...
- **Local development**: pi loads the *installed* copy (`~/.pi/agent/git/github.com/apmantza/pi-webaio` for git installs), **not** your dev checkout — source edits (and even `bin/*.mjs`, resolved relative to the install root) have no effect on a running pi session until the change is merged and the extension is updated. Symptoms of testing against the wrong copy: behavior contradicts the checkout (e.g. a removed flag still appears in `ps`, Chrome launches in the old mode). Debugging aid: compare `ps` flags / `dist` timestamps against the checkout. Node floor is 24 (`engines`): the suite runs under native type-stripping, which older/distro Node builds lack — `npm test` fails there with a cryptic `ERR_NO_TYPESCRIPT`.
...
- **Always-headless Chrome** — removed the `$DISPLAY` auto-detection that popped a visible Chrome window on Linux desktops (`src/google-ai.ts`, `src/paywall.ts` now unconditionally headless; visible only via `GREEDY_SEARCH_VISIBLE=1`); `bin/launch.mjs` passes `--no-sandbox` only as root or with `PI_WEBAIO_NO_SANDBOX=1` (`=0` forces off), so normal users no longer see the "unsupported flag" infobar.
...
- `npm run test:all` → runs all 72 wired suites (1544 tests total, 0 fail, 3 expected skips: a live-network Jina test that skips on external HTTP 403, and opt-in live TLS tests)
...
- `node --experimental-strip-types scripts/smoke-concurrency.mjs [--cold]` → opt-in live Google-lane latency smoke (issue #111 follow-up): cold single → warm single ×2 → warm concurrent ×2, twice; kill Chrome first for `--cold`. Real-world queries only — Google serves genuine zero-result pages for obscure strings. Results logged in `speed.md`.
