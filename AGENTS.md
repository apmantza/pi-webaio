...
├── tests/                    ← 50 suites wired into test:all (1187 tests) + standalone suites (mcp, etc.)
...
- `npm run test:all` → runs all 50 wired suites (1187 tests total, 0 fail, 2 expected skips: a live-network Jina test that skips on external HTTP 403, and an opt-in live TLS test)
