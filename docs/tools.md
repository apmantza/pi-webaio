...
| Tool             | Description                                                                                                                                                                                                                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aio-websearch`  | Search the web using DuckDuckGo, Brave, Yahoo, Bing, Mojeek, and Google in parallel (no API keys required). Returns results with title, URL, snippet, and a per-result `sourceType`, ranked by cross-engine consensus, source type, and optional goggles. 7s cap (per-engine ~4.5s deadline). Google runs via headless Chrome CDP (auto-launched). 10-minute cache. `compact: true` gives one line per result for URL scouting. |
...
| `google`  | `boolean` | `true`  | Also search Google via headless Chrome CDP. Set to `false` to use only the HTTP engines (DDG/Brave/Yahoo/Bing/Mojeek). |
