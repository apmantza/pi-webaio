...
| Tool             | Description                                                                                                                                                                                                                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aio-websearch`  | Search the web using DDG, Brave, Yahoo, Bing, **TinyFish** (API key), and **FireCrawl Keyless** (no key needed) plus Google by default and an opt-in Reddit CDP companion (`reddit: true`, requires Chrome). Returns results with title, URL, snippet, and a per-result `sourceType`, ranked by cross-engine consensus, source type, and optional goggles. Hard 7s response deadline with deterministic partial results. TinyFish and FireCrawl run as parallel providers alongside the HTTP engines. |
| `aio-webfetch`   | Fetch a single URL (or batch of URLs) and convert to markdown with anti-bot TLS fingerprinting. Long content gets a frugal outline + largest-section preview by default; pass `summarize: true` for an AI summary via Google AI Mode. Full file always saved. `tinyfish: true` delegates to TinyFish Fetch API (requires API key). `firecrawl: true` delegates to FireCrawl Keyless Scrape API (no key needed). `query` activates answer mode (top-k relevant chunks); `urls` + `query` gives a multi-source cited answer; `outline: true` returns just a heading outline. Detects PDFs, GitHub repos, and Next.js RSC. Supports `format: "markdown\|html\|text\|json\|raw"`, `chunks` for RAG, auto escalation, and opt-in paywall bypass.     |
