...
| Tool             | Description                                                                                                                                                                                                                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aio-websearch`  | Search the web using DDG, Brave, Yahoo, and Bing plus Google by default and an opt-in Reddit CDP companion (`reddit: true`, requires Chrome) (no API keys required). Returns results with title, URL, snippet, and a per-result `sourceType`, ranked by cross-engine consensus, source type, and optional goggles. Hard 7s response deadline with deterministic partial results (HTTP engines have ~4.5s per-engine deadlines). Google runs via headless Chrome CDP (auto-launched) with its own 3s lane cap (from lane start) so the Google lane never gates the tool deadline, even when paginating multi-page SERPs. `google: false` skips Google only; Reddit runs only when `reddit: true` is passed. 10-minute cache. `compact: true` gives one line per result for URL scouting. |
