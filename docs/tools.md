...
| Tool             | Description                                                                                                                                                                                                                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aio-websearch`  | Search the web using DDG, Brave, Yahoo, Bing, **TinyFish** (API key), **FireCrawl Keyless** (no key), and **Parallel** (API key) plus Google by default and an opt-in Reddit CDP companion (`reddit: true`, requires Chrome). Returns results with title, URL, snippet, and a per-result `sourceType`, ranked by cross-engine consensus, source type, and optional goggles. Hard 7s response deadline with deterministic partial results. TinyFish, FireCrawl, and Parallel run as parallel providers alongside the HTTP engines. |
| `aio-webfetch`   | Fetch a single URL (or batch of URLs) and convert to markdown with anti-bot TLS fingerprinting. Long content gets a frugal outline + largest-section preview by default; pass `summarize: true` for an AI summary via Google AI Mode. Full file always saved. `tinyfish: true` delegates to TinyFish Fetch API (requires API key). `firecrawl: true` delegates to FireCrawl Keyless Scrape API (no key needed). `parallel: true` delegates to Parallel Extract API (requires API key, returns markdown). `query` activates answer mode (top-k relevant chunks); `urls` + `query` gives a multi-source cited answer; `outline: true` returns just a heading outline. Detects PDFs, GitHub repos, and Next.js RSC. Supports `format: "markdown\|html\|text\|json\|raw"`, `chunks` for RAG, auto escalation, and opt-in paywall bypass.     |
...
| `tinyfish`        | `boolean`  | `false`      | Use TinyFish Fetch API instead of the normal extraction pipeline. Requires `TINYFISH_API_KEY` in `~/.piwebaio/config`, `~/.piwebaio/.env`, or env var. |
| `firecrawl`       | `boolean`  | `false`      | Use FireCrawl Keyless Scrape API instead of the normal extraction pipeline. No API key needed — works out of the box (1k free credits/month). |
| `parallel`        | `boolean`  | `false`      | Use Parallel Extract API instead of the normal extraction pipeline. Requires `PARALLEL_API_KEY` in `~/.piwebaio/config`, `~/.piwebaio/.env`, or env var. Handles JS pages and PDFs server-side, returns markdown full content. |
