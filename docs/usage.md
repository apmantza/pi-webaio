...
Google search is on by default (via headless Chrome CDP). Reddit is opt-in: pass `reddit: true` to include the Reddit CDP lane (also requires Chrome). To skip Google:
...
TinyFish, FireCrawl, and Parallel run automatically as parallel bonus providers (FireCrawl needs no key; TinyFish and Parallel need `TINYFISH_API_KEY` / `PARALLEL_API_KEY` in `~/.piwebaio/config` or `~/.piwebaio/.env`). (FireCrawl needs no key; TinyFish needs `TINYFISH_API_KEY` in `~/.piwebaio/config` or `~/.piwebaio/.env`). For fetching a specific URL through one of them instead of the normal pipeline, pass a flag:
...
- Profile directory is tmpdir-resident by default (keeps Google cookies out of persistent storage). Opt-in persistence across reboots/tmp-cleanups — which also makes the first search after a restart warm and far less likely to hit Google's `/sorry/` CAPTCHA — via `PI_WEBAIO_CHROME_PROFILE_DIR=/absolute/path`. A leading `~` expands to the home directory even outside a shell (`~`, `~/…`, `~\…`; `~user` is not supported); relative values are rejected; unset keeps the default.
