...
const testCases = [
  { label: "Standard watch URL", url: "https://www.youtube.com/watch?v=dQw4w9WgXcQ" },
...
for (const tc of testCases) {
  console.log(`  ${tc.label}: ${matchesYouTube(tc.url) ? "✅ match" : "❌ no match"}`);
}
console.log(`  Non-YouTube:   ${matchesYouTube("https://example.com") ? "❌ false positive" : "✅ no match"}`);

console.log("\n=== Extraction tests ===\n");

// Test 1: Standard video
console.log("--- Test 1: Standard video (Rick Astley) ---");
const r1 = await extractYouTube("https://www.youtube.com/watch?v=dQw4w9WgXcQ", noop, noop, noop);
if (!r1?.ok) {
...
const r2 = await extractYouTube("https://youtu.be/dQw4w9WgXcQ", noop, noop, noop);
if (!r2?.ok) {
...
const r3 = await extractYouTube("https://www.youtube.com/watch?v=invalidvideoid1", noop, noop, noop);
if (!r3?.ok) {
...
const r4 = await extractYouTube("https://www.youtube.com/watch?v=dQw4w9WgXcQ", noop, noop, noop, { format: "segments" });
if (!r4?.ok) {
  ...
  const transcriptLines = r4.content.split("## Transcript\n\n")[1]?.split("\n").slice(0, 5);
...
const r5 = await extractYouTube("https://www.youtube.com/watch?v=dQw4w9WgXcQ", noop, noop, noop, { format: "vtt" });
if (!r5?.ok) {
  console.log(`❌ Failed: ${r5?.error ?? "null"}`);
} else {
  console.log(`✅ Title: ${r5.title}`);
  console.log(`   Length: ${r5.content.length} chars`);
  const vttLines = r5.content.split("## Transcript\n\n")[1]?.split("\n").slice(0, 8);
